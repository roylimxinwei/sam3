...
DEFAULT_MODEL_PATH = SCRIPT_DIR / "vit_small.onnx"
...
def _extract_mask_on_black(image_pil, mask, mask_index):
    """
    Extract a single mask as white on black background (binary mask).
    Returns a binary mask image where the segmented area is white (255) 
    ...
    result[mask_np > 0] = [255, 255, 255]  # White for segmented area
    
    return result


def _create_combined_mask(image_pil, masks, selected_indices):
    """
    Create a combined binary mask of all selected items.
    White (255) for all selected segmented areas, black (0) for background.
    """
    if masks is None or len(masks) == 0:
        return None
    
    # Get image dimensions from first mask
    first_mask = masks[0].detach().cpu().numpy().squeeze()
    ...
    for idx in selected_indices:
        if 0 <= idx < len(masks):
            mask_np = masks[idx].detach().cpu().numpy().squeeze()
            combined[mask_np > 0] = 255
    
    return combined


def _save_mask_for_download(mask_array, filename):
    """
    Save a mask array as a PNG file and return the path.
    """
    import tempfile
    
    # Create temp directory if needed
    temp_dir = Path(tempfile.gettempdir()) / "food_masks"
    temp_dir.mkdir(exist_ok=True)
    
    filepath = temp_dir / filename
    Image.fromarray(mask_array).save(filepath)
...
def confirm_selection(cache, selected_indices):
    ...
    if not cache:
        return [], "Run segmentation first.", [], None, []
    ...
    if not selected_indices:
        return [], "No items selected. Click on items to select them first.", [], None, []
    ...
    # Generate individual mask images (white on black) with predictions
    individual_images = []
    prediction_rows = []
    download_files = []
    ...
    for idx in sorted(selected_indices):
        if 0 <= idx < len(masks):
            # Create white binary mask on black background
            ...
            # Save individual mask for download
            safe_label = pred_label.replace(" ", "_").replace("/", "-")
            mask_path = _save_mask_for_download(mask_img, f"mask_{idx}_{safe_label}.png")
            download_files.append(mask_path)
    
    # Create combined mask of all selected items
    combined_mask = _create_combined_mask(image, masks, sorted(selected_indices))
    combined_path = None
    if combined_mask is not None:
        combined_path = _save_mask_for_download(combined_mask, "combined_mask.png")
    
    status = f"✅ Classified {len(selected_indices)} '{prompt}' item(s). Masks ready for download."
    return individual_images, status, prediction_rows, combined_path, download_files
...
with gr.Blocks(title="SAM3 Click-to-Select Segmentation") as segmentation_demo:
    ...
    with gr.Row():
        with gr.Column():
            ...
            gr.Markdown("*White binary masks on black background with classification*")
            ...
            gr.Markdown("### 📥 Download Masks")
            with gr.Row():
                combined_mask_file = gr.File(
                    label="Combined Mask (all selected items)",
                    file_count="single"
                )
                individual_mask_files = gr.File(
    ...
    confirm_btn.click(
        confirm_selection,
        inputs=[det_cache, selected_state],
        outputs=[final_output, confirm_status, prediction_results, combined_mask_file, individual_mask_files]
    )
